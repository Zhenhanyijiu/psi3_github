...
namespace osuCrypto
{
	...
	void OPPRFSender::init(u32 opt, u64 numParties, u64 setSize, u64 statSec,
						   u64 inputBitSize,
						   ...
						   block seed, bool isOtherDirection)
	{
			...
			(otRecv.hasBaseOts() == false && isOtherDirection))
		{
			...
			if (!isOtherDirection) //if it is not dual, number extend OT is 128
				dualBaseOtCount = 0;
			//iknp接收方，recvChoice是选择哪一列
			BitVector recvChoice(baseOtCount + dualBaseOtCount);
			recvChoice.randomize(mPrng);
			std::vector<block> recvBaseMsg(baseOtCount + dualBaseOtCount);
			//iknp接收方
			IknpOtExtReceiver kosRecv;
			printf(">>>我是iknp03接收方，会提取512个(e1...en)一次性随机密钥...\n");
			printf(">>>baseOtCount=%ld,dualBaseOtCount=%ld,recvBaseMsg.size:%ld个\n",
				   baseOtCount, dualBaseOtCount, recvBaseMsg.size()); //512,0,512
			kosRecv.setBaseOts(baseMsg);
			//根据iknp recvchoice 会生成512个一次性随机密钥
			kosRecv.receive(recvChoice, recvBaseMsg, mPrng, chl0);
			// cout << "iknp 接收方选择的比特串为:" << recvChoice << ",长度:"
			//  << recvChoice.size() << endl;
			// we now have a bunch of recv OTs, lets seed the NcoOtExtSender
			BitVector kcoSendBaseChoice; //kkrt选择的随机数
			kcoSendBaseChoice.copy(recvChoice, 0, baseOtCount);
			ArrayView<block> kcoSendBase(
				recvBaseMsg.begin(),
				recvBaseMsg.begin() + baseOtCount);
			printf("===>>我是kkrt16协议发送方(但我是iknp的接收方，消息单个)...\n");
			cout << "===>>recvChoice       :" << recvChoice << ",长度:"
				 << recvChoice.size() << endl;
			cout << "===>>kcoSendBaseChoice:" << kcoSendBaseChoice << ",长度:"
				 << recvChoice.size() << endl;
			otSend.setBaseOts(kcoSendBase, kcoSendBaseChoice);
			// cout << "===>>>kkrt发送方base:" << kcoSendBase<<endl;
			for (int i = 0; i < kcoSendBase.size(); i++)
			{
				// cout << "===>>>kkrt发送方base,i:" << i << "," << kcoSendBase[i] << endl;
			}
			for (int i = 0; i < recvBaseMsg.size(); i++)
			{
				if (i < 16)
				{
					cout << "===>>>iknp接收方recover,i:" << i << "," << recvBaseMsg[i] << endl;
				}
			}
		...
		printf("######(opprfsend)>>numRecvThreads:%ld,numSendThreads:%ld\n", numRecvThreads, numSendThreads);
		...
		printf("######(opprfsend)>>numRecvThreads:%ld,numSendThreads:%ld\n",
			   numRecvThreads, numSendThreads); //0,0
	...
	void OPPRFSender::getOPRFkeys(u64 IdxParty, binSet &bins, const std::vector<Channel *> &chls, bool isOtherDirectionGetOPRF)
	{
		...
		if (bins.mOpt == 0)
		{
			printf(">>>>>>OPPRFSender::getOPRFkeys>>>>>>>\n");
	...
	{
		printf(">>>(OPPRFSender::getOPRFkeysSeperatedandTable)IdxP:%ld\n", IdxP);
			...
			thrds[tIdx] = std::thread([&, tIdx, seed]()
									  {
										  ...
										  if (tIdx == 0)
											  gTimer.setTimePoint("online.send.thrdStart");

										  auto &chl = *chls[tIdx];

										  if (tIdx == 0)
											  gTimer.setTimePoint("online.send.insert");
...
#pragma region compute Send Bark - OPRF
										  ...
										  if (tIdx == 0)
											  gTimer.setTimePoint("online.send.OT");
								...
								std::cout << "s-"<<bIdx <<", "<< inputIdx << ": " << sendMask << std::endl;*/
										  ...
										  if (tIdx == 0)
											  gTimer.setTimePoint("online.send.otSend.finalOPRF");
...
#pragma region compute Recv Bark - OPRF
										  ...
										  if (isOtherDirectionGetOPRF)
										  {
											  ...
											  if (tIdx == 0)
												  gTimer.setTimePoint("online.send.otRecv.finalOPRF");
...
#pragma region sendShare
#if 1
										  if (tIdx == 0)
											  gTimer.setTimePoint("online.send.sendShare");
										  ...
										  for (auto bIdxType = 0; bIdxType < 2; bIdxType++)
										  {
											  ...
											  if (tIdx == 0)
												  gTimer.setTimePoint("online.send.masks.init.step");
				...
				chl.asyncSend(std::move(sendMaskBuff));*/
										  ...
										  if (tIdx == 0)
											  gTimer.setTimePoint("online.send.finalMask");
#endif
#pragma endregion
									  });
		}
...
#pragma region sendShare
							...
							std::cout << "\n----" << y << std::endl;
										  ...
										  if (tIdx == 0)
											  gTimer.setTimePoint("online.compute x y");
#endif
#pragma endregion
									  });
		}
			...
			thrds[tIdx] = std::thread([&, tIdx, seed]()
									  {
										  ...
										  if (tIdx == 0)
											  gTimer.setTimePoint("online.recv.recvShare");
