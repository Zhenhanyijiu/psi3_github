...
inline void zeroXOR_party(u64 myIdx, u64 nPartiesZeroXor, u64 nParties,
						  const std::vector<std::vector<Channel *>> chls,
						  std::vector<block> inputSet,
						  const std::vector<block> inputSet2ZeroXOR,
						  std::vector<u64> &mIntersection,
						  u64 type_okvs, u64 type_security)
{
	printf("===>>进入zeroXOR_party...\n");
	u64 chl_idx_shift = nParties - nPartiesZeroXor; //3-3=0
	//leaderId为2号节点
	u64 leaderIdx = nPartiesZeroXor - 1; //3-1=2
...
#pragma region setup
	...
	if (myIdx == 1)
	{
		//这里优化，保证prng不一样，opprf发送方
		block tmpbloc = prng.get<block>();
	}
...
#pragma endregion
	...
	for (u64 j = 0; j < nPartiesZeroXor; ++j)
	{
		//假设myIdx=0，seed=00,01,02
		//假设myIdx=1，seed=10,11,12
		//假设myIdx=2，seed=20,21,22
		//每个节点都生成3个prng,
		mSeedPrng[j].SetSeed(mSeeds[myIdx][j]);
	}

	if (myIdx == leaderIdx) //leader==2
	{
		//三个block 向量分配空间
	...
	printf("===>>从简单  hash，获取 otCountSend:%ld\n", otCountSend);
	printf("===>>从布谷鸟hash，获取 otCountRecv:%ld\n", otCountRecv);
	...
	if (myIdx != leaderIdx)
	{
		...
		send.init(bins.mOpt, nPartiesZeroXor, setSize, psiSecParam, bitSize,
				  chls[leaderIdx + chl_idx_shift],
				  otCountSend,
				  otSend[leaderIdx],
				  otRecv[leaderIdx],
		...
		for (u64 pIdx = 0; pIdx < pThrds.size(); ++pIdx)
		{
			pThrds[pIdx] = std::thread([&, pIdx]()
						...
						std::cout << IoStream::unlock;*/
											   ...
											   recv[pIdx].init(bins.mOpt, nPartiesZeroXor,
															   setSize, psiSecParam, bitSize,
															   chls[pIdx + chl_idx_shift],
															   otCountRecv, otRecv[pIdx],
															   otSend[pIdx], ZeroBlock, false);
										   }
									   });
		}
		for (u64 pIdx = 0; pIdx < pThrds.size(); ++pIdx)
...
#pragma region setup
	...
	printf("===>>这里 okvsTableSize 的大小:%ld\n", okvsTableSize);
	std::string name("tpsi");
	BtIOService ios(0);
...
#pragma endregion
	...
	{
		printf("===>>当threshold小于nParties-1时才走这里...\n");
		//====================================
	...
	if (myIdx < nParties && myIdx >= party_t_id) //for zeroXOR
	{
		...
		std::cout << IoStream::unlock;*/
		printf("===>>1号节点和2号节点，运行这里zeroXOR_party...\n");
		zeroXOR_party(myIdx - party_t_id, threshold, nParties, chls, inputSet,
					  inputSet2ZeroXOR, mIntersection, TableOPPRF, secSemiHonest);
