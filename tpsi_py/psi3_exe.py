...
class Psi3PocCsv(object):
    ...
    def __get_key_list_dict(self):
        start_csv = time.time()
        dfa_id = self.__get_id_data_frame()
        ...
        # time.sleep(10*60)
        # 加一列，hash_id
        hash_id_tmp = pd.DataFrame(dfa_id['id']).applymap(
            lambda x: md5(x.encode('utf-8')).digest())
        hash_id_tmp['id_src'] = dfa_id['id']
        # 加一列,用于分桶的标志
        hash_id_tmp['hash_id_key'] = hash_id_tmp['id'].apply(
            divide_to_16iterms)
        # 加一列，索引列
    ...
    def get_psi3_ids_results(self) -> tuple:
