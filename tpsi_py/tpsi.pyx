...
# cdef extern from "tpsiout.h" namespace "osuCrypto":
cdef extern from "tpsiout.h":
    ...
    int tpsi_process(u64_t p_idx, u64_t n_parties, u64_t threshold, u64_t set_size,
                     vector[vector[char]] ip_array,
                     vector[vector[u32_t]] port_array,
                     vector[vector[u8_t]] data_set,
                     vector[u64_t] *psi_results_output)

def psi3_process_by_boost(p_idx: int, set_size: int,
                          ip_array: list, port_array: list, data_set: np.array):
    if len(ip_array) != 3:
    ...
    cdef n_parties = 3
    cdef threshold = 3
    cdef ret = tpsi_process(p_idx, n_parties, threshold, set_size, ip_array_c, port_array_c, data_set_c, &psi_results)
    if ret != 0:
