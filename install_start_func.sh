...
### Extract LinBox sources ###
	...
	if [ -f linbox-${STABLE_LB}.tar.gz ] ; then
	    ...
	    wget --no-check-certificate git://github.com/linbox-team/linbox/releases/download/v${STABLE_LB}/linbox-${STABLE_LB}.tar.gz.${MD5SUFF} >/dev/null 2>&1 || die
	    [ -f linbox-${STABLE_LB}.tar.gz.${MD5SUFF} ] || die
	    cool| tee -a linbox-auto-install.log
	    echo -ne "${BEG}"
	    md5sum -c linbox-${STABLE_LB}.tar.gz.${MD5SUFF} || die
	else
	    wget git://github.com/linbox-team/linbox/releases/download/v${STABLE_LB}/linbox-${STABLE_LB}.tar.gz >/dev/null 2>&1 || die
	...
	OK=0 ;
	git clone --depth 1 git://github.com/linbox-team/linbox.git 2>&1 >/dev/null && OK=1
	[ "$OK" = "1" ] &&  cool | tee -a linbox-auto-install.log || die
	cd linbox &&  cool   || die
    fi
...
if [ "$STABLE_VAR" = "true" ]; then
	...
	wget --no-check-certificate git://github.com/linbox-team/givaro/releases/download/v${STABLE_GIVARO}/givaro-${STABLE_GIVARO}.tar.gz.${MD5SUFF} >/dev/null 2>&1 || die
	[ -f givaro-${STABLE_GIVARO}.tar.gz.${MD5SUFF} ] || die
	cool| tee -a ../linbox-auto-install.log
	echo -ne "${BEG}"
	md5sum -c givaro-${STABLE_GIVARO}.tar.gz.${MD5SUFF} || die
    else
	wget --no-check-certificate git://github.com/linbox-team/givaro/releases/download/v${STABLE_GIVARO}/givaro-${STABLE_GIVARO}.tar.gz >/dev/null 2>&1 || die
    ...
    OK=0 ;
    git clone --depth 1 git://github.com/linbox-team/givaro.git 2>&1 >/dev/null && OK=1
    [ "$OK" = "1" ] &&  cool | tee -a ../linbox-auto-install.log || die
fi
...
if [ "$STABLE_VAR" = "true" ]; then
	...
	wget --no-check-certificate git://github.com/linbox-team/fflas-ffpack/releases/download/v${STABLE_FFLAS}/fflas-ffpack-${STABLE_FFLAS}.tar.gz.${MD5SUFF} >/dev/null 2>&1 || die
	[ -f fflas-ffpack-${STABLE_FFLAS}.tar.gz.${MD5SUFF} ] &&  cool || die
	cool
	echo -ne "${BEG}"
	md5sum -c fflas-ffpack-${STABLE_FFLAS}.tar.gz.${MD5SUFF} || die
    else
	wget --no-check-certificate git://github.com/linbox-team/fflas-ffpack/releases/download/v${STABLE_FFLAS}/fflas-ffpack-${STABLE_FFLAS}.tar.gz >/dev/null 2>&1 || die
    ...
    OK=0 ;
    git clone --depth=1 git://github.com/linbox-team/fflas-ffpack.git 2>&1 >/dev/null && OK=1
    [ "$OK" = "1" ] &&  cool | tee -a ../linbox-auto-install.log || die
fi
...
if [ "$OPENBLAS_VAR" = "true" ]; then
	...
	if [ -f v${STABLE_OPENBLAS}.tar.gz ] ; then
	    ...
	    wget --no-check-certificate git://github.com/xianyi/OpenBLAS/archive/v${STABLE_OPENBLAS}.tar.gz >/dev/null 2>&1 || die
	    [ -f v${STABLE_OPENBLAS}.tar.gz ] &&  cool || die
	fi
    else
	OK=0 ;
	git clone --depth=1 git://github.com/xianyi/OpenBLAS.git 2>&1 >/dev/null && OK=1
	[ "$OK" = "1" ] &&  cool | tee -a ../linbox-auto-install.log || die
    fi
fi
...
if [ "$BLIS_VAR" = "true" ]; then
	...
	if [ -f ${STABLE_BLIS}.tar.gz ] ; then
	    ...
	    wget --no-check-certificate git://github.com/flame/blis/archive/${STABLE_BLIS}.tar.gz >/dev/null 2>&1 || die
	    [ -f ${STABLE_BLIS}.tar.gz ] &&  cool || die
	fi
    else
	OK=0 ;
	git clone --depth=1 git://github.com/flame/blis.git 2>&1 >/dev/null && OK=1
	[ "$OK" = "1" ] &&  cool | tee -a ../linbox-auto-install.log || die
    fi
fi
...
##编译ntl
# echo "编译ntl==============>"
# cd build/ntl-11.4.3/src/
# make clean
# make ./ntl.a CXXFLAGS="-fPIC -shared"
# mv ./ntl.a /tmp/lib/libntl.a
# cd -
