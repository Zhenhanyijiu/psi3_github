...
namespace osuCrypto
{
    ...
    void KkrtNcoOtReceiver::init(u64 numOtExt)
    {
        ...
        std::array<std::array<block, superBlkSize>, 128> t0; //1024*128
        std::array<std::array<block, superBlkSize>, 128> t1; //1024*128

        // we are going to process OTs in blocks of 128 * superblkSize messages.
        //144*1024,每1024位作为一个超级block
        u64 numSuperBlocks = ((numOtExt + 127) / 128 + superBlkSize - 1) / superBlkSize;
        u64 numCols = mGens.size(); //512对
        printf("#########(in kkrt recv)mGens.size():%ld对,传入的参数numOtExt:%ld,numSuperBlocks:%ld\n",
               mGens.size(), numOtExt, numSuperBlocks); //512对，147000
        ...
        mT0 = std::move(MatrixView<block>(numOtExt, numCols / 128)); //4列，147000行
        mT1 = std::move(MatrixView<block>(numOtExt, numCols / 128)); //4列，147000行
        printf("#######kkrtrecv mT0.size[%ld,%ld]\n", mT0.size()[0], mT0.size()[1]);
        ...
        //144*1024,每1024位作为一个超级block，numSuperBlocks==144
        for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
        {
            ...
            u64 stopIdx = doneIdx + std::min(u64(128) * superBlkSize, numOtExt - doneIdx);

            for (u64 i = 0; i < numCols / 128; ++i) //循环4次
            {
                //i=0,1,2,3
                ...
                // each 128 bits wide.
                sse_transpose128x1024(t0); //变成1024行，128bits
                sse_transpose128x1024(t1); //变成1024行，128bits
                ...
                for (u64 rowIdx = doneIdx, j = 0; rowIdx < stopIdx; ++j) //1024/128=8次
                {
        ...
        block &val)
    {
        ...
        for (u64 i = 0; i < mT0.size()[1]; ++i)
        {
            ...
            t1Val[i] = choice[i] ^ t0Val[i] ^ t1Val[i];
        }
